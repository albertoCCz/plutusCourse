...
payTrace x y = do
    ...
    callEndpoint @"pay" h PayParams{ppRecipient=pkh, ppLovelace=x}
    void $ Emulator.waitNSlots 1

    callEndpoint @"pay" h PayParams{ppRecipient=pkh, ppLovelace=y}
    void $ Emulator.waitNSlots 1
  where
    pkh = pubKeyHash $ walletPubKey $ Wallet 2
