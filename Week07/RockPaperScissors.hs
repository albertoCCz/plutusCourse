...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just _)) (Reveal nonce c1) _ =
    case c1 of
        Rock     -> sha2_256 (nonce `concatenate` bsRock')     == bs
        Paper    -> sha2_256 (nonce `concatenate` bsPaper')    == bs
        Scissors -> sha2_256 (nonce `concatenate` bsScissors') == bs
...
firstGame fp = do
    ...
    let game   = Game
        ...
        c      = fpChoice fp
        bs     = sha2_256 $ fpNonce fp `concatenate` (case c of
            Rock     -> bsRock
            Paper    -> bsPaper
            Scissors -> bsScissors)
