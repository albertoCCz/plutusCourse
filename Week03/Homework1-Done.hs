...
mkValidator dat () ctx =
    traceIfFalse  "beneficiary1's signature missing"  checkSig1                 &&
    traceIfFalse  "deadline has passed"               deadlineNotPassed             ||
    traceIfFalse  "beneficiary2's signauture missing" checkSig2                 &&
    traceIfFalse  "deadline has not passed"           (not deadlineNotPassed)
  where
    info :: TxInfo
    ...
    deadlineNotPassed = to (deadline dat) `contains` txInfoValidRange info
